...
"""
#from controversy import polarization_index, network_disagreement_index, echo_chamber_index, controversy_index
import numpy as np
import pandas as pd
import os
import heapq
...
def calculate_DJ_row(A, u, in_degrees, out_d):
    ...
    # row u
    EPS = 10E-10
    #POPULATION = A.shape[0]
    AAt = np.multiply(A[u, :] * A, np.logical_not(A[u, :])) # tengo solo i non vicini, metto i vicini a zero
    out_d = A[u, :].sum()      # salva in memoria
    #print(out_d.shape, in_degrees.shape)
    den = (- AAt + out_d + in_degrees)
    #print(AAt, out_d, in_degrees, den)
