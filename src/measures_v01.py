...
def compute_cont_correlation_neighbors(opinions, G):
    ...
    #print(avg_neigh_opinions)
    #return np.corrcoef(avg_neigh_opinions, opinions)[0,1]
...
######################################################### Old Measures
# def compute_initial_stats(G, x):
#     A = nx.to_numpy_array(G)
#     ndi_val = network_disagreement_index(A,x)
#     return compute_controversy_measures(0,A,x,G.number_of_edges(), ndi_val,G)
#
#
#
# def compute_controversy_measures(edges_added, adjacency_matrix ,opinion_distribution, number_of_edges, ndi_value, network):
#     DELTA = 0.3
#     row_time_step_zero = {"edges_added":edges_added}
#     #add ndi
#     ndi = ndi_value
#     row_time_step_zero["ndi"] = ndi
#     #add ndi norm
#     row_time_step_zero[ "ndi_norm"] = ndi/number_of_edges
#     #add polarization index
#     row_time_step_zero["pi"] = polarization_index(opinion_distribution)
#     #add controversy at delta index
#     row_time_step_zero["controversy_at_"+str(DELTA)] = controversy_index(G = network, x = opinion_distribution, DELTA = DELTA)
#     #compute cho chamber
#     row_time_step_zero["ec_index"] = echo_chamber_index(G = network, x = opinion_distribution, DELTA = DELTA)
#     return row_time_step_zero
